...
class BlackjackGame:
    ...
    def dealer_action(self, output=False):
        while self.hand_value(self.dealer_hand) < 17:
            ...
            if output:
                print("Dağıtıcı kartları çekiyor ve şunları alıyor:", self.format_cards(self.dealer_hand), self.hand_value(self.dealer_hand))
...
def main():
    print("Dağıtıcı kartları gösteriyor:", game.format_cards(game.dealer_hand[:1]))

    status = "continue"
    while status == "continue":
        print(game.format_cards(game.player_hand), game.hand_value(game.player_hand))
        action = input("Bir işlem girin (hit/stay): ")
    ...
    if status == "continue":
        print("Dağıtıcı kartları:", game.format_cards(game.dealer_hand), game.hand_value(game.dealer_hand))
        game.dealer_action()
